    ...
    return Organisation.findOneAsync({
      ...
      .then((event) => {
        ...
        if(event.synchronous === true) {
          console.log("SUBSCRIBING TO THIS ENDPOINT");
          console.log(`${event.eventName}::completed`);
          this._receiver.subscribe(event, `${event.eventName}::completed`)
            .then((response) => {
