  ...
  processRequest: function (request, response) {
    ...
    BBPromise.try(function parseBody() {
      ...
      .then(function publishEvent(ev) {
        logger.info({eventId:ev.messageId,application:ev.applicationId,environment:ev.environment},'publishing event');
        logger.debug({event:ev},'event');
        return EventBroker.publish(ev).then(function () {
          return ev;
        });
      }).then(function generateResponse(ev) {

        logger.info({eventId:ev.messageId,application:ev.applicationId,environment:ev.environment},'sending event response');
        logger.debug({event:ev},'event');
